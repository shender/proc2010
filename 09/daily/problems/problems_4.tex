...
\taskpic{К свободному концу нити, прикреплённой к стене и перекинутой
  ...
  \begin{tikzpicture}
    ...
    \begin{tikzpicture}[interface/.style={postaction={draw,decorate,decoration={border,angle=-45,amplitude=0.2cm,segment length=1.5mm}}}]
      \draw[interface] (-3,3) -- ++(0,-3) -- ++ (4,0) -- ++(0,-3);
      \draw[thick] (-1.5,0) rectangle ++(2,1) node[midway] {$m_0$};
      \node[circle,draw] (c) at (1.2,1.7) [minimum size=0.2] {};
      \draw (2.5,0.5) -- (tangent cs:node=c,point={(2.5,0.5)},solution=1);
      \draw[thick] (1.2,1.7) -- (0.5,1);
      \draw (-3,1.9) -- ++(4.2,0);
      \shadedraw[shading=ball] (2.5,0.5) circle (0.15);
      \draw[dashed] (1.4,2) -- ++(0,-2);
      \draw (1.4,1.3) arc (-90:-28:0.3) node[below=1] {$\alpha$};
    \end{tikzpicture}};
  \end{tikzpicture}
